 ...
 * 日本語をローマ字表記にした書き方をしない。※日記だったら、nikkiじゃなくてdiary。
...
さて、`li`などブロック要素辺りはクラス名を付けて作業することに慣れていると思いますが、  
`a`タグや`img`タグはどうでしょうか？  
このまま上記のやり方を踏襲すると、  
...
何か別の指定をしなければならくなった場合にクラス名を使用する。  
`.coverGirl-list-item > a.somethingLink`  
てな具合に、`a`と`img`タグは通常クラス名を付けなくても良しとします。
